...
- [ingest/](./ingest) Download data from NCBI, clean, format, curate it, and assign clades.
- [phylogenetic/](./phylogenetic) Subsample data and make phylogenetic trees for use in nextstrain.
...
The Washington focused WNV build uses the earliest USA-based sequence [NC_009942](https://www.ncbi.nlm.nih.gov/nuccore/NC_009942.1) as the reference sequence. This sequence was isolated from the first WNV outbreak in the Americas, which occured in New York in 1999.
...
### Root Selection
The Global and the Washington focused WNV phylogenetic trees use different root node sequences.

The Global WNV build uses the global midpoint as the root node.

The Washington focused WNV build uses [AF481864](https://www.ncbi.nlm.nih.gov/nuccore/AF481864) as the root node sequence. This is the sequence that is most closely related to the sequences isolated from New York in 1999. 
_Hadfield J, Brito AF, Swetnam DM, Vogels CBF, Tokarz RE, Andersen KG, Smith RC, Bedford T, Grubaugh ND. Twenty years of West Nile virus spread and evolution in the Americas visualized by Nextstrain. PLoS Pathog. 2019 Oct 31;15(10):e1008042. doi: 10.1371/journal.ppat.1008042. PMID: 31671157; PMCID: PMC6822705.
...
* To modify the minimum length of nucleotide sequence in the WNV global build enter the desired threshold in the --min-length <MIN_LENGTH> parameter that is listed in the [defaults/config.yaml](./phylogenetic/defaults/config.yaml) file
* To modify the minimum length of nucleotide sequence in the WNV Washington focused build enter the desired threshold in the --min-length <MIN_LENGTH> parameter that is listed in the [washington-state/config.yaml](./phylogenetic/build-configs/washington-state/config.yaml) file.
